...
unsigned long long pxt4_file_write_iter_total_time, pxt4_file_write_iter_total_count;

static unsigned long get_current_cpu_id(void *elem)
{
	return ((struct task_struct *) elem)->cpu;
}

static void print_pxt4_file_write_iter_metrics(unsigned long index, const char *name, unsigned long long count, int percentage)
{
	printk("cpu[%lu] called pxt4_file_write_iter() %llu times (%d%%)\n", index, count, percentage);
}

DEFINE_DS_MONITORING(pxt4_file_write_iter_dm, get_current_cpu_id, NULL, print_pxt4_file_write_iter_metrics)
...
static ssize_t pxt4_file_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	...
	calclock(t, &pxt4_file_write_iter_total_time, &pxt4_file_write_iter_total_count);
	return ret;
}
