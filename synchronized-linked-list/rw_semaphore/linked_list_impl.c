...
// define your rw_semaphore here
DECLARE_RWSEM(list_rw_semaphore);
...
void *add_to_list(int thread_id, int range_bound[])
{
	...
	for (i = range_bound[0]; i <= range_bound[1]; i++) {
		...
		down_write(&list_rw_semaphore);
		list_add_tail(&new->list, &my_list);
		up_write(&list_rw_semaphore);
